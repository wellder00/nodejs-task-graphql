...
const collectBy = <ItemType>(items: ItemType[], keySelector: (item: ItemType) => string) =>
...
export const indexBy = <ItemType>(items: ItemType[], keySelector: (item: ItemType) => string) =>
  ...
  export const initializeDataLoaders = (dbClient: PrismaClient) => ({
    ...
    fetchSubscriptionsToUser: new DataLoader(async (ids: ReadonlyArray<User['id']>) => {
      ...
      const subscriptionsGroupedByUser = collectBy(usersWithSubscriptions, (user) => user.id);
  
      return ids.map(
        (userId) =>
          subscriptionsGroupedByUser[userId]?.flatMap((record) => record.subscribedToUser.map((sub) => sub.subscriber)) ?? [],
      );
    }),
    ...
    fetchUserSubscriptions: new DataLoader(async (ids: ReadonlyArray<User['id']>) => {
      ...
      return ids.map(
        (userId) =>
          subscribersGroupedByUser[userId]?.flatMap((record) => record.userSubscribedTo.map((subscription) => subscription.author)) ?? [],
      );
    }),
  });
